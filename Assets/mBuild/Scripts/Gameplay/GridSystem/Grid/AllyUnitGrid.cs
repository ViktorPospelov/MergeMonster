...
public class AllyUnitGrid : UnitGrid
{
    ...
    public void AddNewUnit(int unitLevel = 1)
    {
        if (TryFindFreeSlot(out GridSlot slot))
        {
            ...
            unit.Initialize(unitLevel, GetTeam());
            slot.SetUnit(unit);

            OnGridStateChanged?.Invoke();
            YandexGame.savesData.playerUnits = GetLevelsData();
    ...
    protected override GridSlot GetSlotsPrefab() => ServiceLocator.GetService<Bootstrap>().AllyGridSlotPrefab;
